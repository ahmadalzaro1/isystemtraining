-- Drop and recreate the get_workshops_week function to include description and max_capacity
...
CREATE OR REPLACE FUNCTION public.get_workshops_week(p_start date, p_end date, p_levels text[] DEFAULT NULL::text[], p_categories text[] DEFAULT NULL::text[], p_query text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, name text, instructor text, date date, time_text text, skill_level text, category text, spots_remaining integer, location text, description text, max_capacity integer)
 LANGUAGE sql
